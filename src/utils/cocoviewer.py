...
Usage:
    python cocoviewer.py --i PATH_TO_IMAGES --a PATH_TO_ANN_FILE
"""
import argparse
import colorsys
import json
...
def coco_viewer(annotations: str, images: str):
    print_info("Starting...")
    ...
    if not annotations:
    ...
    elif not os.path.exists(annotations) or not os.path.isfile(annotations):
        ...
        try:
            ...
            images = json.loads(open(annotations).read())["image_root"]
        except KeyError:
...
def main():
    ...
    coco_viewer(args.annotations, args.images)
